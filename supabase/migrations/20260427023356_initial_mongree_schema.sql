...
-- Keep existing helper functions private if they already exist in this project.
do $$
begin
  if exists (
...
declare
  ...
  found_id uuid;
begin
  normalized_name := nullif(trim(tag_name), '');
  if normalized_name is null then
    raise exception 'tag_name must not be empty';
  end if;
...
revoke all on function public.get_or_create_tag_id(text) from anon, public;
grant execute on function public.get_or_create_tag_id(text) to authenticated, service_role;
...
-- Emotions and tags are readable reference data.
...
for select using (deleted_at is null and (is_private = false or auth.uid() = user_id));
...
-- Diary children follow diary visibility for reads and diary ownership for writes.
create policy "diary_emotions_select_visible" on public.diary_emotions
for select using (exists (select 1 from public.diaries d where d.id = diary_id and d.deleted_at is null and (d.is_private = false or d.user_id = auth.uid())));
create policy "diary_emotions_insert_owner" on public.diary_emotions
for insert to authenticated with check (exists (select 1 from public.diaries d where d.id = diary_id and d.user_id = auth.uid()));
create policy "diary_emotions_delete_owner" on public.diary_emotions
for delete to authenticated using (exists (select 1 from public.diaries d where d.id = diary_id and d.user_id = auth.uid()));
...
for insert to authenticated with check (auth.uid() = user_id and exists (select 1 from public.diaries d where d.id = diary_id and d.deleted_at is null and d.is_private = false));
create policy "diary_likes_delete_own" on public.diary_likes
for delete to authenticated using (auth.uid() = user_id);

create policy "comments_select_visible" on public.comments
for select using (deleted_at is null and exists (select 1 from public.diaries d where d.id = diary_id and d.deleted_at is null and (d.is_private = false or d.user_id = auth.uid())));
create policy "comments_insert_authenticated_visible" on public.comments
for insert to authenticated with check (auth.uid() = user_id and exists (select 1 from public.diaries d where d.id = diary_id and d.deleted_at is null and (d.is_private = false or d.user_id = auth.uid())));
...
for select using (exists (select 1 from public.comments c join public.diaries d on d.id = c.diary_id where c.id = comment_id and c.deleted_at is null and d.deleted_at is null and (d.is_private = false or d.user_id = auth.uid())));
create policy "comment_likes_insert_own_visible" on public.comment_likes
for insert to authenticated with check (auth.uid() = user_id and exists (select 1 from public.comments c join public.diaries d on d.id = c.diary_id where c.id = comment_id and c.deleted_at is null and d.deleted_at is null and (d.is_private = false or d.user_id = auth.uid())));
